when not declared HARULIB_MODINT:
  const HARULIB_MODINT* = 1
  ...
  when declared(Mod):
    ...
    proc initMint*(a:SomeInteger):Mint = initModInt(a, Mod)
    #proc `$`*(a:Mint):string = $(a.v)
    converter toMint*(a:SomeInteger):Mint = initModInt(a, Mod)
